...
| 现代控件外观 | Qt 原生（Fusion）的立体滚动条 / 三角箭头 / 凸起微调按钮 / 带描边 GroupBox 全部被主题 QSS 接管：扁平、无边框、紧凑，对齐 VSCode；工具栏 / 活动栏图标用 VSCode 官方 **Codicons**（随程序分发，染色跟随主题）；界面字号与编辑器**同一个值**（设置里的「字号」），`Ctrl+=` / `Ctrl+-` / `Ctrl+0` 是**整个界面**缩放（图标 / 间距 / 栏宽 / 行高一起变，不只是文字） |
...
| 源代码管理面板 | 仿 VSCode 的 SCM 视图：提交信息框 + 「提交」按钮（回车亦可，有信息且有更改时才可点）、「更改」分组带数量与折叠、每行显示「**文件名 + 弱化的父目录** + 变更徽标」。点文件名直接打开；**打开面板不发任何远端请求**（复用文件树已有的 Git 快照），右上角刷新等价 `Shift+F5`。提交 = `git add -A` + `git commit -m`（**全部暂存后提交**，暂无逐个暂存区），成功后自动刷新状态与标记 |
...
| Markdown 预览 | `.md` 文件可开「编辑器 \| 预览」可拖动分屏（`Ctrl+Shift+V` 或「视图 → 切换 Markdown 预览」）：左侧照旧编辑、右侧实时渲染（编辑后 300ms 节流刷新），标题 / 表格 / 代码块 / 引用 / 链接都认，颜色随主题；用 Qt 原生渲染，**不引入第三方解析器**（终端聚焦时 `Ctrl+Shift+V` 仍是粘贴） |
| 文件同步 | 本地编辑、`Ctrl+S` 经 SFTP 上传；上传采用临时文件 + 原子重命名，避免中断损坏远端源码 |
| Git 行级标记 | Gutter 彩色标记相对 `HEAD` 的新增 / 修改 / 删除行；**点击标记可看该处与上一版的差异**（红底旧文本 / 绿底新文本的浮层，Esc 关闭，不额外发远端请求）；非仓库时状态栏提示 |
...
| `Ctrl+Shift+V` | 切换 Markdown 预览 | | （终端聚焦时该组合是粘贴） |
